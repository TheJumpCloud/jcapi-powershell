// Code generated by Microsoft (R) AutoRest Code Generator (autorest: 3.0.6282, generator: {generator})
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

namespace JumpCloud.SDK.V2.Models
{
    using static JumpCloud.SDK.V2.Runtime.Extensions;

    /// <summary>Directory</summary>
    public partial class Directory :
        JumpCloud.SDK.V2.Models.IDirectory,
        JumpCloud.SDK.V2.Models.IDirectoryInternal
    {

        /// <summary>Backing field for <see cref="Id" /> property.</summary>
        private string _id;

        /// <summary>The ObjectID of the directory.</summary>
        [JumpCloud.SDK.V2.Origin(JumpCloud.SDK.V2.PropertyOrigin.Owned)]
        public string Id { get => this._id; set => this._id = value; }

        /// <summary>Backing field for <see cref="Name" /> property.</summary>
        private string _name;

        /// <summary>The name of the directory.</summary>
        [JumpCloud.SDK.V2.Origin(JumpCloud.SDK.V2.PropertyOrigin.Owned)]
        public string Name { get => this._name; set => this._name = value; }

        /// <summary>Backing field for <see cref="OAuthStatus" /> property.</summary>
        private JumpCloud.SDK.V2.Models.IDirectoryOAuthStatus _oAuthStatus;

        /// <summary>the expiry and error status of the bearer token</summary>
        [JumpCloud.SDK.V2.Origin(JumpCloud.SDK.V2.PropertyOrigin.Owned)]
        public JumpCloud.SDK.V2.Models.IDirectoryOAuthStatus OAuthStatus { get => (this._oAuthStatus = this._oAuthStatus ?? new JumpCloud.SDK.V2.Models.DirectoryOAuthStatus()); set => this._oAuthStatus = value; }

        /// <summary>Backing field for <see cref="Type" /> property.</summary>
        private string _type;

        /// <summary>The type of directory.</summary>
        [JumpCloud.SDK.V2.Origin(JumpCloud.SDK.V2.PropertyOrigin.Owned)]
        public string Type { get => this._type; set => this._type = value; }

        /// <summary>Creates an new <see cref="Directory" /> instance.</summary>
        public Directory()
        {

        }
    }
    /// Directory
    public partial interface IDirectory :
        JumpCloud.SDK.V2.Runtime.IJsonSerializable
    {
        /// <summary>The ObjectID of the directory.</summary>
        [JumpCloud.SDK.V2.Runtime.Info(
        Required = true,
        ReadOnly = false,
        Description = @"The ObjectID of the directory.",
        SerializedName = @"id",
        PossibleTypes = new [] { typeof(string) })]
        string Id { get; set; }
        /// <summary>The name of the directory.</summary>
        [JumpCloud.SDK.V2.Runtime.Info(
        Required = true,
        ReadOnly = false,
        Description = @"The name of the directory.",
        SerializedName = @"name",
        PossibleTypes = new [] { typeof(string) })]
        string Name { get; set; }
        /// <summary>the expiry and error status of the bearer token</summary>
        [JumpCloud.SDK.V2.Runtime.Info(
        Required = false,
        ReadOnly = false,
        Description = @"the expiry and error status of the bearer token",
        SerializedName = @"oAuthStatus",
        PossibleTypes = new [] { typeof(JumpCloud.SDK.V2.Models.IDirectoryOAuthStatus) })]
        JumpCloud.SDK.V2.Models.IDirectoryOAuthStatus OAuthStatus { get; set; }
        /// <summary>The type of directory.</summary>
        [JumpCloud.SDK.V2.Runtime.Info(
        Required = true,
        ReadOnly = false,
        Description = @"The type of directory.",
        SerializedName = @"type",
        PossibleTypes = new [] { typeof(string) })]
        string Type { get; set; }

    }
    /// Directory
    internal partial interface IDirectoryInternal

    {
        /// <summary>The ObjectID of the directory.</summary>
        string Id { get; set; }
        /// <summary>The name of the directory.</summary>
        string Name { get; set; }
        /// <summary>the expiry and error status of the bearer token</summary>
        JumpCloud.SDK.V2.Models.IDirectoryOAuthStatus OAuthStatus { get; set; }
        /// <summary>The type of directory.</summary>
        string Type { get; set; }

    }
}