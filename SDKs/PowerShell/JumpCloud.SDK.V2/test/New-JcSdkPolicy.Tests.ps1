...
Describe 'New-JcSdkPolicy' {
    ...
    It 'Create' {
        # Create a policy
        $WindowsPolicy = Get-JcSdkPolicyTemplate | ? { $_.OSMetaFamily -eq 'windows' } | Select-Object -Last 1
        $LinuxPolicy = Get-JcSdkPolicyTemplate | ? { $_.OSMetaFamily -eq 'linux' } | Select-Object -Last 1
        $DarwinPolicy = Get-JcSdkPolicyTemplate | ? { $_.OSMetaFamily -eq 'darwin' } | Select-Object -Last 1
