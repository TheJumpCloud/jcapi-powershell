...
Describe 'New-JcSdkBulkUserState' -Tag:(""){
    ...
    It 'Create' {
        ...
        { New-JcSdkBulkUserState -StartDate (Get-Date).AddDays(1) -UserIds $user.Id -State "SUSPENDED" } | Should -Not -Throw
        $SuspendedState = New-JcSdkBulkUserState -StartDate (Get-Date).AddDays(1) -UserIds $user.Id -State "SUSPENDED"
        $SuspendedState.State | Should -Be "SUSPENDED"
        # Set the user to be activated
        $ActivatedState = New-JcSdkBulkUserState -StartDate (Get-Date).AddDays(1) -UserIds $user.Id -State "ACTIVATED"
        $ActivatedState.State | Should -Be "ACTIVATED"
