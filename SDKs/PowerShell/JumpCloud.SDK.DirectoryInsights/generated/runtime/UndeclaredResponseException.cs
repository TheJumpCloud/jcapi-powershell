    ...
    public class RestException : Exception, IDisposable
    {
        ...
        public RestException(System.Net.Http.HttpResponseMessage response)
        {
            ...
            try
            {
                ...
                var json = JumpCloud.SDK.DirectoryInsights.Runtime.Json.JsonNode.Parse(ResponseBody) as JumpCloud.SDK.DirectoryInsights.Runtime.Json.JsonObject;

                // see if there is an error block in the body
                json = json.Property("error") ?? json;

                { Code = If(json?.PropertyT<JumpCloud.SDK.DirectoryInsights.Runtime.Json.JsonString>("code"), out var c) ? (string)c : (string)StatusCode.ToString(); }
                { message = If(json?.PropertyT<JumpCloud.SDK.DirectoryInsights.Runtime.Json.JsonString>("message"), out var m) ? (string)m : (string)Message; }
                { Action = If(json?.PropertyT<JumpCloud.SDK.DirectoryInsights.Runtime.Json.JsonString>("action"), out var a) ? (string)a : (string)Action; }
            }
#if DEBUG
