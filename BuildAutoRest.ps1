            ...
            If (($UpdatedSpec -and $env:USERNAME -eq 'VssAdministrator') -or $RunLocal)
            {
                ...
                If ($TestModule)
                {
                    If (-not [System.String]::IsNullOrEmpty($env:JCApiKey) -and -not [System.String]::IsNullOrEmpty($env:JCOrgId))
                    {
                        ...
                        # $testModuleContent.Replace("# Load the latest Az.Accounts installed", "# Load the latest Az.Accounts installed`n  If(-not (Get-InstalledModule -Name Az.Accounts -ErrorAction 'SilentlyContinue')){Install-Module -Name Az.Accounts -Force}`n  If(-not (Get-Module -Name Az.Accounts -ListAvailable)){Import-Module -Name Az.Accounts -Force}") | Set-Content -Path:($testModulePath)
                        # Test module
