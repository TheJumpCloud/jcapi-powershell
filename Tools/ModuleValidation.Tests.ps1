...
Describe -Tag:('ModuleValidation') 'Module Validation' -ForEach $modulesToValidate {
    BeforeAll {
        # set the module name
        $moduleName = $_
    }

    It "validates the module version against the gallery based on the release type for <_>" {
        ...
        if ($env:RELEASE_TYPE) {
            switch ($env:RELEASE_TYPE.ToLower()) {
                ...
                'manual' {
                    Write-Host "Skipping changelog validation because release type is not 'major', 'minor', or 'patch'."
                }
            }
        }
    }

    It "validates the changelog has the correct new version and today's date for <_>" {
    ...
    It "ensures the changelog does not contain placeholder content for <_>" {
        $changelogPath = Join-Path $PSScriptRoot '..' "$moduleName.md"
