...
$modulePsd1 = Get-Item -Path (Join-Path $PSScriptRoot './JumpCloud.SDK.DirectoryInsights.psd1')
...
try {
  ...
  Invoke-Pester -Path $testFolder -PassThru | Export-NUnitReport -Path "D:\a\1\s\AutoRest\SDKs\PowerShell\JumpCloud.SDK.DirectoryInsights\test\JumpCloud.SDK.DirectoryInsights-TestResults.xml"
}
Finally
{
  if ($TestMode -ne 'playback') {
