    ...
    public class RestException : Exception, IDisposable
    {
        ...
        public RestException(System.Net.Http.HttpResponseMessage response)
        {
            ...
            try
            {
                ...
                var json = JumpCloudApiSdkV2.Runtime.Json.JsonNode.Parse(ResponseBody) as JumpCloudApiSdkV2.Runtime.Json.JsonObject;
                { Code = If(json?.PropertyT<JumpCloudApiSdkV2.Runtime.Json.JsonString>("code"), out var c) ? (string)c : (string)StatusCode.ToString(); }
                { message = If(json?.PropertyT<JumpCloudApiSdkV2.Runtime.Json.JsonString>("message"), out var m) ? (string)m : (string)Message; }
                { Action = If(json?.PropertyT<JumpCloudApiSdkV2.Runtime.Json.JsonString>("action"), out var a) ? (string)a : (string)Action; }
            }
#if DEBUG
